...
void NJtree(darray<double> & _distmatx,nbstr & nbmatx)
{
	 ...
	 while(distmatx.getrnum()>3)			//stop until 3 neighbours left
	 {
		 ...
		 for(int i=0;i<Dkarr.size();++i)
		 {
			 ...
			 {
				 Q=distmatx(i,j)-Dkarr[i]-Dkarr[j];
//the nature of finding a neighbour is to find the minimum Q,which maximize the common path linking i,j and other nodes
				 if(Qmin>Q)
				 {
					 Qmin=Q;
					 marki=i;
...
////////////////////////////////////////////////////////////////construct neighbour 
		nbmatx._marr1.push_row_i(idarray[marki],'i');
		nbmatx._marr2.push_row_i(idarray[markj],'i');
		nbmatx._nbsize+=1;
////////////////////////////////////////////////////////////// 
...
////////////////////////////////////////////////////////////////update sequence IDs
		...
		idarraynew.push_row_i(idarray[marki],'i');
		idarraynew.push_to_row_i(idarraynew.getrnum()-1,idarray[markj],'i');
		for(int i=0;i<idarray.getrnum();++i)
		{
			if(i!=marki && i!=markj)
				idarraynew.push_row_i(idarray[i],'i');
		}
		idarray=idarraynew;
		idarraynew.clear();
	}                                                         //while loop stopped here
////////////////////////////////////////////////////////////////process ID information of the final three nodes
	nbmatx._marr1.push_row_i(idarray[1],'i');
	nbmatx._marr2.push_row_i(idarray[2],'i');
	nbmatx._nbsize+=1;
	nbmatx._marr1.push_row_i(idarray[0],'i');
	nbmatx._marr2.push_row_i(idarray[1],'i');
	nbmatx._marr2.push_to_row_i(nbmatx._nbsize,idarray[2],'i');
	nbmatx._nbsize+=1;
